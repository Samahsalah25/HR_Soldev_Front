...
import { Bell, X, CheckCircle, FileWarning, CalendarDays, CreditCard, Gift, MinusCircle, AlertTriangle } from "lucide-react";
import {
  getNotifications,
...
const TYPE_CONFIG = {
  compliance: { icon: FileWarning, bg: "bg-red-50 border-red-100", iconColor: "text-red-500" },
  leave: { icon: CalendarDays, bg: "bg-blue-50 border-blue-100", iconColor: "text-blue-500" },
  loan: { icon: CreditCard, bg: "bg-amber-50 border-amber-100", iconColor: "text-amber-500" },
  bonus: { icon: Gift, bg: "bg-blue-50 border-blue-100", iconColor: "text-blue-500" },
  deduction: { icon: MinusCircle, bg: "bg-amber-50 border-amber-100", iconColor: "text-amber-500" },
  default: { icon: AlertTriangle, bg: "bg-slate-50 border-slate-100", iconColor: "text-slate-500" },
};

function getTypeConfig(notificationType) {
  return TYPE_CONFIG[notificationType] || TYPE_CONFIG.default;
}
...
function normalizeNotification(n) {
  return {
    ...
    createdAt: n.create_date,
  };
}

// unreadCount و setUnreadCount جايين من فوق (Layout) عشان الجرس في الهيدر
// يقدر يعرف العدد برضو، مش بس البانل نفسه
export default function NotificationPanel({ isOpen, onClose, unreadCount, setUnreadCount }) {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      ...
      const list = (res?.data || []).map(normalizeNotification);
      setNotifications(list);
      setUnreadCount(res?.unread_count ?? 0);
    } catch (err) {
      console.error("تعذّر تحميل الإشعارات:", err);
    } finally {
      setLoading(false);
    }
  }, [setUnreadCount]);
  ...
  const handleMarkOneAsRead = async (id) => {
    ...
    try {
      ...
      console.error("تعذّر تحديد الإشعار كمقروء:", err);
      loadNotifications();
    }
  };
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      console.error("تعذّر تحديد كل الإشعارات كمقروءة:", err);
  ...
  const handleNotificationClick = (notif) => {
    if (!notif.isRead) handleMarkOneAsRead(notif.id);
    if (notif.actionUrl) {
        ...
        <div className="max-h-96 overflow-y-auto">
          {loading ? (
            ...
            notifications.map((notif) => {
              const { icon: Icon, bg, iconColor } = getTypeConfig(notif.notificationType);
              return (
                <div
                  key={notif.id}
                  onClick={() => handleNotificationClick(notif)}
                  className={`flex items-start gap-3 px-4 py-3 border-b border-border last:border-0 cursor-pointer ${
                    notif.isRead ? "opacity-60" : bg
                  }`}
                >
                  <div className={`mt-0.5 p-1.5 rounded-lg bg-white/80 ${iconColor}`}>
                    <Icon className="w-3.5 h-3.5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-foreground">{notif.title}</p>
