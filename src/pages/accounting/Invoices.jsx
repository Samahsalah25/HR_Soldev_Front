...
//           // لأن invoice.lines أصلاً جاية بالـ id من الشاشة الرئيسية
...
// أنواع الحسابات المسموح بيها لبنود فاتورة العميل (إيرادات).
...
const INCOME_ACCOUNT_TYPES = ["income", "income_other"];
...
function InvoiceForm({ invoice, customers, accounts, taxes, paymentTerms, products, journals, onSave, onClose }) {
  ...
  // فلترة الحسابات على أنواع الإيرادات فقط، عشان أودو ميرفضش الفاتورة
  // وقت الاعتماد بسبب استخدام حساب من نوع غير متوافق (زي أصول أو التزامات).
  const activeAccounts = (() => {
    const active = accounts.filter((a) => a.is_active);
    const filtered = active.filter((a) => INCOME_ACCOUNT_TYPES.includes(a.account_type));
    if (filtered.length === 0) {
      console.warn(
        `[InvoiceForm] لا توجد حسابات بنوع إيراد (${INCOME_ACCOUNT_TYPES.join(", ")}). ` +
    ...
    try {
      ...
      setAccounts(
        (accs || []).map((item) => ({
          ...
          account_name: item.name_ar ?? item.account_name,
          is_active: item.active ?? item.is_active,
          // نوع الحساب زي ما راجع من الـ API (asset_current, income...)
          // بيتفلتر عليه بعدين في InvoiceForm عشان نعرض حسابات إيراد بس.
