...
// حساب كشف الراتب الكامل
// ───────────────────────────────────────────────
export function calcPayslip(employee, absenceDays = 0, lateMinutes = 0, overtimeHours = 0, loanDeductionParam = 0) {
  const { basic_salary, housing_allowance, transport_allowance, food_allowance,
    communication_allowance, other_allowances, is_saudi } = employee;
  ...
  // خصم قسط السلفة
  const loanDeduction = loanDeductionParam || 0;
...
// ───────────────────────────────────────────────
/**
 * حساب مكافأة نهاية الخدمة بناءً على التصنيف القانوني الكامل (departure_type)
 *
 * @param {number} basicSalary - الراتب الأساسي
 * @param {number} serviceYears - عدد سنوات الخدمة
 * @param {string} departureType - أحد القيم: resignation | termination | expiry |
 *                                  article_80 | article_81 | force_majeure | death_disability
 * @param {string} contractType - "محدد المدة" | غير ذلك (غير محدد المدة)
 * @param {boolean} isFemaleException - استثناء الموظفات: يمنح كامل المكافأة حتى لو كان النوع "استقالة"
 */
export function calcEndOfService(basicSalary, serviceYears, departureType, contractType, isFemaleException = false) {
  const fullReward = basicSalary * serviceYears;
  let reward = 0;

  // الأنواع اللي بتاخد المكافأة كاملة دايمًا، بغض النظر عن سنوات الخدمة أو نوع العقد
  // (إنهاء من الشركة / انتهاء عقد / مادة 81 / ظروف قاهرة / وفاة أو عجز)
  const fullRewardTypes = ["termination", "expiry", "article_81", "force_majeure", "death_disability"];

  if (departureType === "article_80") {
    // المادة 80: فصل تأديبي — لا يستحق الموظف أي مكافأة
    reward = 0;
  } else if (fullRewardTypes.includes(departureType)) {
    reward = fullReward;
  } else if (departureType === "resignation") {
    if (isFemaleException) {
      // استثناء الموظفات: كامل المكافأة حتى لو كان النوع استقالة
      reward = fullReward;
    } else if (contractType === "محدد المدة") {
      // عقد محدد المدة + استقالة = نصف المكافأة (حسب الاتفاق التعاقدي المعتمد)
      reward = fullReward * 0.5;
    } else {
      // عقد غير محدد المدة + استقالة = التدرج حسب سنوات الخدمة
      if (serviceYears < 2) reward = 0;
      else if (serviceYears < 5) reward = fullReward * (1 / 3);
      else if (serviceYears < 10) reward = fullReward * (2 / 3);
      else reward = fullReward;
  ...
  // أي departureType غير معروف/فاضي => reward تفضل صفر (تحوّط بدل افتراض قيمة كاملة بالغلط)

  return {
    serviceYears,
    departureType,
    isFemaleException: !!isFemaleException,
    fullReward: Math.round(fullReward * 100) / 100,
    finalReward: Math.round(reward * 100) / 100,
  };
}
