...
import { ConfirmDialogProvider } from '@/components/ui/confirm-dialog';
import UserNotRegisteredError from '@/components/UserNotRegisteredError';
import api from './api/axios.js'; // TODO: تأكد إن ده مسار ملف الـ Odoo axios instance عندك (odooClient)
// Add page imports here
import Layout from './components/Layout';
import Dashboard from './pages/Dashboard';
import Employees from './pages/Employees';
...
const AuthenticatedApp = () => {
  ...
  useEffect(() => {
    ...
    if (!user) { setTerminatedChecked(true); return; }
    // admins are never blocked (case-insensitive check — الـ role بيرجع "Admin" مش "admin")
    if (user.role?.toLowerCase() === 'admin') { setTerminatedChecked(true); return; }

    // لا يوجد email في بيانات اليوزر، نتحقق عبر EmployeeNumber بدلاً منه
    if (!user.EmployeeNumber) { setTerminatedChecked(true); return; }

    // TODO: تأكد إن "/employees" هو المسار الصحيح، وإن الحقل في الـ response اسمه EmployeeNumber فعلاً
    api.get('/employees').then(({ data: emps }) => {
      const matched = emps.filter(e => e.EmployeeNumber === user.EmployeeNumber);
      const hasActive = matched.some(e => e.status !== "مُنهي الخدمة");
      const allTerminated = matched.length > 0 && !hasActive;
      setTerminatedBlock(allTerminated);
      setTerminatedChecked(true);
