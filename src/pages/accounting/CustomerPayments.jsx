...
// ===== فورم / تفاصيل سند دفع عميل =====
function PaymentForm({ payment, customers, onBack, onSave }) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6" dir="rtl">
      ...
      <div className="bg-card border rounded-2xl p-6 shadow-sm space-y-6">
        <h2 className="text-2xl font-bold">{payment?.voucher_number || "سند دفع جديد"}</h2>
...
export default function CustomerPayments() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const [vRes, custs] = await Promise.all([
        getVouchers(),
        getCustomers().catch(() => []),
      ]);
      setPayments(vRes?.receipt_vouchers || []);
      setCustomers(custs);
      setSelected((prev) => (prev ? (vRes?.receipt_vouchers || []).find((p) => p.id === prev.id) || null : prev));
    } catch (err) {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-5" dir="rtl">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">متابعة وسداد فواتير وحركات العملاء المالية</p>
        </div>
