...
function PolicyForm({ policy, onSave, onClose }) {
  const [form, setForm] = useState({
    title: "", title_en: "", category: "الإجازات والغياب",
    description: "", file_url: "", version: "", effective_date: "", is_active: true,
    ...(policy || {}),
  });
  ...
  const handleSave = async () => {
    setSaving(true);
    if (policy?.id) await base44.entities.CompanyPolicy.update(policy.id, form);
    else await base44.entities.CompanyPolicy.create(form);
    onSave();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" dir="rtl">
      <div className="bg-card rounded-2xl border border-border w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h3 className="font-bold text-foreground">{policy ? "تعديل السياسة" : "إضافة سياسة جديدة"}</h3>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-muted"><X className="w-4 h-4" /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">عنوان السياسة *</label>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">Policy Title (EN)</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">الإصدار</label>
              <input value={form.version} onChange={e => set("version", e.target.value)} placeholder="1.0"
                className="w-full px-3 py-2 text-sm border border-border rounded-lg bg-background text-foreground focus:outline-none" />
            </div>
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">تاريخ السريان</label>
          ...
          <div className="space-y-2">
            ...
            {form.file_url ? (
              <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                ...
                <span className="text-xs text-green-700 flex-1 truncate">تم رفع الملف</span>
                <a href={form.file_url} target="_blank" rel="noopener noreferrer"
                  className="text-xs text-blue-600 hover:underline flex items-center gap-1">
                  <ExternalLink className="w-3 h-3" />فتح
                </a>
                <button onClick={() => set("file_url", "")} className="text-red-500 hover:text-red-700">
              ...
              <label className="flex flex-col items-center gap-2 p-4 border-2 border-dashed border-border rounded-lg cursor-pointer hover:bg-muted/30 transition-colors">
                ...
                <span className="text-sm text-muted-foreground">{uploading ? "جاري الرفع..." : "اضغط لرفع ملف PDF"}</span>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm text-foreground">منشور للموظفين</span>
          </label>
        </div>
...
export default function Policies() {
  ...
  const load = async () => {
    const ps = await base44.entities.CompanyPolicy.list("-created_date");
    setPolicies(ps);
    setLoading(false);
  };
  ...
  const deletePolicy = async (id) => {
    if (confirm("هل أنت متأكد من حذف هذه السياسة؟")) {
      await base44.entities.CompanyPolicy.delete(id);
      load();
    }
  };
  ...
  return (
    <div className="p-6 space-y-5 max-w-5xl mx-auto" dir="rtl">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-card rounded-xl border border-border p-4 text-center">
          <p className="text-2xl font-bold text-secondary">{policies.filter(p => p.file_url).length}</p>
                  ...
                  <div className="flex flex-col gap-1 flex-shrink-0">
                    {pol.file_url && (
                      <a href={pol.file_url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-700 rounded-lg text-xs hover:bg-blue-100">
                        <ExternalLink className="w-3 h-3" />فتح
                      </a>
                    )}
