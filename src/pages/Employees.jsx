...
// مودال تأكيد الحذف — بديل عن confirm() الافتراضي بتاع المتصفح
function DeleteConfirmDialog({ employee, onCancel, onConfirm, loading }) {
  if (!employee) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" dir="rtl">
      <div className="bg-card w-full max-w-sm rounded-xl border border-border shadow-lg p-5 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              ...
              <span className="font-medium text-foreground">{employee.full_name_ar}</span>؟
...
const { toast } = useToast();
  ...
  // بيانات القائمة (GET /employees) ناقصة بعض الحقول (زي المشروع) —
  // نجيب بيانات الموظف كاملة (GET /employees/:id) قبل ما نفتح فورم التعديل
  const handleEditClick = async (emp) => {
    setLoadingEdit(true);
    try {
      const full = await getEmployeeById(emp.id);
      setEditEmployee(normalizeEmployee(full));
    } catch (err) {
  ...
  // بينفذ الحذف الفعلي بعد ما المستخدم يأكد من المودال
    ...
    <div className="p-6 space-y-5 max-w-7xl mx-auto" dir="rtl">
                  ...
                  <tr key={emp.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        {canEdit && (
                          <button onClick={() => handleEditClick(emp)} disabled={loadingEdit} title="تعديل"
                            className="p-1.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-secondary disabled:opacity-50">
                            <Edit className="w-4 h-4" />
                          </button>
                        )}
                        {canDelete && (
                          <button onClick={() => handleDeleteClick(emp)} title="حذف"
      ...
      {viewEmployee && (
        <EmployeeDetail
          ...
          onEdit={(emp) => { setViewEmployee(null); handleEditClick(emp); }}
        />
      )}
