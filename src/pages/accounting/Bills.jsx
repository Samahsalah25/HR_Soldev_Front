...
// أنواع الحسابات المسموح بيها لبنود فاتورة المورد (مصروفات).
// لو مفيش حساب مطابق، بنرجع لعرض كل الحسابات النشطة (fallback دفاعي)
// وبنطبع تحذير في الكونسول بالأنواع الفعلية المتاحة عشان تظبط القايمة دي.
const EXPENSE_ACCOUNT_TYPES = ["expense", "expense_direct_cost", "expense_depreciation"];
...
function BillForm({ bill, vendors, accounts, taxes, paymentTerms, products, journals, onSave, onClose }) {
  ...
  const [lines, setLines] = useState(
    bill?.lines?.length
      ? bill.lines.map((l) => ({
          // *** الإصلاح الأهم ***: لازم نحافظ على id بند الفاتورة هنا،
          // لأن bill.lines أصلاً جاية بالـ id من الشاشة الرئيسية
          // (lines: (fullBill.invoice_line_ids || []).map((l) => ({ ...l }))).
          // من غيرها، buildPayload مستحيل يعرف إن السطر ده موجود
          // أصلاً في أودو، وهيتبعت كأنه سطر جديد (create) بدل update،
          // وده هيسبب تكرار البنود في الفاتورة.
          id: l.id,
          product_id: l.product_id || "",
          name: l.name || "",
          quantity: l.quantity ?? 1,
  ...
  // بنود الفاتورة اللي اتمسحت من الفورم وكانت أصلاً موجودة في الباك
  // اند (عندها id حقيقي). بتتبعت في الآخر بصيغة { id, _delete: true }.
  ...
  // *** الإصلاح ***: لو البند اللي بيتمسح عنده id حقيقي (جاي من الباك
  // اند)، بنسجله في deletedLineIds عشان يتبعت لاحقًا كـ
  // { id, _delete: true }، بدل ما يختفي بس من الـ state المحلي ويفضل
  // موجود في الفاتورة الفعلية في أودو.
  const removeLine = (i) => {
    if (lines.length <= 1) return;
    const removed = lines[i];
    setLines((ls) => ls.filter((_, idx) => idx !== i));
    if (removed.id) {
      setDeletedLineIds((ids) => [...ids, removed.id]);
    }
  };
  ...
  // فلترة الحسابات على أنواع المصروفات فقط، عشان أودو ميرفضش الفاتورة
  // وقت الاعتماد بسبب استخدام حساب من نوع غير متوافق (زي أصول أو التزامات).
  const activeAccounts = (() => {
    const active = accounts.filter((a) => a.is_active);
    const filtered = active.filter((a) => EXPENSE_ACCOUNT_TYPES.includes(a.account_type));
    if (filtered.length === 0) {
      console.warn(
        `[BillForm] لا توجد حسابات بنوع مصروف (${EXPENSE_ACCOUNT_TYPES.join(", ")}). ` +
        `الأنواع المتاحة فعليًا: ${[...new Set(active.map((a) => a.account_type))].join(", ")}`
      );
      return active;
    }
    return filtered;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // بنود نشطة: كل سطر لسه موجود في الفورم. لو عنده id حقيقي بنبعته
      // عشان يبقى update، ولو مالوش (سطر جديد) منبعتوش id خالص.
      const activeLines = lines.map((l) => {
        const line = {
          product_id: l.product_id ? Number(l.product_id) : undefined,
          name: l.name,
          quantity: Number(l.quantity) || 0,
      ...
      // بنود اتمسحت وكانت أصلاً موجودة (لها id حقيقي) — بتتبعت بصيغة
      // { id, _delete: true } عشان تتمسح فعليًا من الفاتورة في أودو.
      const deletedLines = deletedLineIds.map((id) => ({ id, _delete: true }));
      ...
      const payload = {
        ...
        invoice_payment_term_id: form.invoice_payment_term_id ? Number(form.invoice_payment_term_id) : undefined,
        journal_id: Number(form.journal_id),
        invoice_line_ids: [...activeLines, ...deletedLines],
      };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" dir="rtl">
      <div className="bg-card rounded-2xl border border-border w-full max-w-5xl shadow-2xl max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h3 className="font-bold flex items-center gap-2">
            <FileText className="w-5 h-5 text-primary" />
            {isEdit ? `تعديل الفاتورة ${bill.name || ""}` : "فاتورة مورد جديدة"}
                    ...
                    <tr key={l.id ?? i} className="border-b border-border last:border-0">
                      ...
                      <td className="px-2 py-2">
                        <button
                          ...
                          title={lines.length <= 1 ? "لازم يفضل بند واحد على الأقل في الفاتورة" : "حذف البند"}
    ...
    try {
      ...
      setAccounts(
        (accs || []).map((item) => ({
          ...
          account_name: item.name_ar ?? item.account_name,
          is_active: item.active ?? item.is_active,
          // نوع الحساب زي ما راجع من الـ API (asset_current, expense, income...)
          // بيتفلتر عليه بعدين في BillForm عشان نعرض حسابات مصروف بس.
