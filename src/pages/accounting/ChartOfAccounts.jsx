...
//   "حقوق الملكية": "property_rights",
//   "إيرادات": "revenues",
//   "مصروفات": "expenses",
// };

// const SUBCATEGORIES = {
//   "أصول متداولة": "trading_assets",
//   "أصول ثابتة": "fixed_assets",
//   "أصول أخرى": "other_assets",

//   "التزامات متداولة": "trading_commitments",
//   "التزامات طويلة الأجل": "long_term_commitments",

//   "رأس المال": "capital",

//   "إيرادات تشغيلية": "operational_revenue",
//   "إيرادات أخرى": "other_revenue",

//   "مصروفات تشغيلية": "operational_expenses",
//   "مصروفات إدارية": "administrative_expenses",
//   "مصروفات مالية": "financial_expenses",
//   "أخرى": "other",
// };
...
//       account_type: ACCOUNT_TYPES[form.account_type],
//       subcategory: SUBCATEGORIES[form.account_category],
//       name_ar: form.account_name,
//       name_en: form.account_name_en,
//       natural_balance: BALANCE[form.normal_balance],
//       parent_id: form.parent_account_id
//         ? Number(form.parent_account_id)
//         : undefined,
//       opening_amount: Number(form.opening_balance),
//       opening_amount_type: BALANCE[form.opening_balance_type],
//       parent_no_restrictions: form.is_parent,
...
//           ? "أصول"
//           : item.account_type === "commitments"
//           ? "التزامات"
//           : item.account_type === "property_rights"
//           ? "حقوق الملكية"
//           : item.account_type === "revenues"
//           ? "إيرادات"
//           : "مصروفات",

//       account_category: item.subcategory_label?.split(" / ")[1] || "",

//       parent_account_id: item.parent_id || "",
//       parent_account_code: item.parent_code || "",

//       is_parent: item.parent_no_restrictions,
//       normal_balance:
//         item.natural_balance === "debt" ? "مدين" : "دائن",

//       opening_balance: item.opening_amount,
//       opening_balance_type:
//         item.opening_amount_type === "debt" ? "مدين" : "دائن",
...
// ====== قواميس التحويل (منقولة برة الكومبوننت عشان متتعادش كل render) ======
const ACCOUNT_TYPES = {
  "أصول": "assets",
  "التزامات": "commitments",
  "حقوق الملكية": "property_rights",
...
const ACCOUNT_TYPES_REVERSE = Object.fromEntries(
  Object.entries(ACCOUNT_TYPES).map(([ar, en]) => [en, ar])
);
...
const SUBCATEGORIES_REVERSE = Object.fromEntries(
  Object.entries(SUBCATEGORIES).map(([ar, en]) => [en, ar])
);
...
const BALANCE_REVERSE = Object.fromEntries(
  Object.entries(BALANCE).map(([ar, en]) => [en, ar])
);
...
function AccountForm({ account, accounts, onSave, onClose }) {
  ...
  const handleTypeChange = (type) => {
    set("account_type", type);
    set("normal_balance", DEFAULT_NORMAL_BALANCE[type] || "مدين");
    // لما يتغير النوع، نتأكد إن التصنيف الفرعي يبقى متوافق مع النوع الجديد
    set("account_category", (CATEGORIES[type] || [])[0] || "");
  };

  const handleParentSelect = (id) => {
    const parent = accounts.find(a => a.id === id || String(a.id) === id);
    if (parent) {
      set("parent_account_id", id);
      set("parent_account_code", parent.account_code);
      set("level", (parent.level || 1) + 1);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // لازم ننده على onSave بعد نجاح الحفظ عشان الأب يعمل load() ويقفل الفورم
      onSave();
    } catch (err) {
      console.error("خطأ أثناء حفظ الحساب:", err);
      setError("حصل خطأ أثناء حفظ الحساب، حاول تاني.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const load = async () => {
    try {
      ...
      const mapped = data.map((item) => ({
        ...
        account_type: ACCOUNT_TYPES_REVERSE[item.account_type] || "أصول",

        // بنستخدم reverse mapping حقيقي بدل التخمين من subcategory_label
        account_category: SUBCATEGORIES_REVERSE[item.subcategory] || "",

        parent_account_id: item.parent_id || "",
        parent_account_code: item.parent_code || "",

        is_parent: item.parent_no_restrictions,
        normal_balance: BALANCE_REVERSE[item.natural_balance] || "مدين",

        opening_balance: item.opening_amount,
        opening_balance_type: BALANCE_REVERSE[item.opening_amount_type] || "مدين",
