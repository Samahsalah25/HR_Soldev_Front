...
import { UserPlus, Plus, X, Save, Search, CheckCircle, XCircle, Eye, ChevronRight, Calendar, Star, FileText, ExternalLink } from "lucide-react";
...
useEffect(() => {
 ...
 const isHR =
  ...
  const addInterview = () => {
    ...
    setNewInterview({ interview_type:"مقابلة HR", interview_date:"", interviewer_name:"", interviewer_email:"", score:"", notes:"", result:"في الانتظار" });
  };
  ...
  const handleSave = async () => {
    ...
    await base44.entities.JobApplication.update(app.id, {
      interviews, final_result: finalResult, hr_notes: hrNotes
    });
    onSave();
  };
                    ...
                    {isHR ? (
                      <select value={iv.interviewer_email || ""} onChange={e=>{
                        ...
                        setInterviews(prev => prev.map((item, i) => i===idx ? {
                          ...
                          interviewer_name: u?.full_name || e.target.value
                        } : item));
            ...
            <div className="border-2 border-dashed border-primary/30 rounded-xl p-4 space-y-3 bg-primary/5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1 col-span-2">
                  <label className="text-xs text-muted-foreground">المحاور *</label>
                  <select value={newInterview.interviewer_email} onChange={e => {
                    const u = users.find(x => x.email === e.target.value);
                    setNewInterview(f => ({ ...f, interviewer_email: e.target.value, interviewer_name: u?.full_name || e.target.value }));
                  }}
                    className="w-full px-2 py-1.5 text-xs border border-border rounded-lg bg-background focus:outline-none">
...
const pendingJobs = jobs.filter(
  ...
  const saveMyInterviewResult = async (app, ivIdx, field, value) => {
    const updated = app.interviews.map((iv, i) =>
      i === ivIdx ? { ...iv, [field]: field === "score" ? +value : value } : iv
    );
    await base44.entities.JobApplication.update(app.id, { interviews: updated });
    load();
  };
      ...
      {activeTab==="my-interviews" && (
        <div className="space-y-4">
          ...
          {myInterviewApps.length === 0 ? (
            ...
            const myInterviews = app.interviews
              .map((iv, idx) => ({ ...iv, _idx: idx }))
